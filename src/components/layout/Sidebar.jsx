      ...
      <aside className={cn(
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-slate-100 dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-2 text-primary dark:text-blue-500 font-bold text-xl">
             ...
             <span>Varavu Selavu</span>
          </div>
          <button className="md:hidden text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white p-1 rounded-lg transition-colors" onClick={onClose}>
             <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 shrink-0">
          {user ? (
            <div className="flex items-center justify-between px-3 py-2 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg group transition-colors" onClick={logout}>
              <div className="flex items-center gap-3">
                <img src={user.photoURL || `https://ui-avatars.com/api/?name=${user.email}`} alt="User" className="w-8 h-8 rounded-full border border-slate-200 dark:border-slate-700" />
                <div className="text-sm overflow-hidden">
                  <p className="font-medium text-slate-900 dark:text-slate-100 truncate max-w-[100px]">{user.displayName || 'User'}</p>
            ...
            <button onClick={loginWithGoogle} className="w-full flex justify-center items-center py-2 px-4 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
              Sign In with Google
            </button>
          )}
        </div>
