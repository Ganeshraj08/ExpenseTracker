...
export function Routines() {
  ...
  const validCategories = useMemo(() => {
    ...
    categories.forEach((c) => {
      flat.push(c.name);
      if (c.subcategories)
        c.subcategories.forEach((s) => flat.push(`${c.name}: ${s}`));
    });
    return flat;
  }, [categories]);
  ...
  const handleMagicRoutineAdd = async (transcript) => {
    ...
    setAiParsing(true);
    try {
      const result = await parseTransactionNLP(
        transcript,
      ...
      if (dataArray.length > 0) {
        ...
        setTransactions((prev) => {
          if (prev.length === 1 && !prev[0].amount && !prev[0].description)
            return newTxs;
          return [...prev, ...newTxs];
        });
        setNlpText("");
  ...
  const openModal = (routine = null) => {
    ...
    if (routine) {
      ...
      const txs =
        routine.transactions && routine.transactions.length > 0
  ...
  const handleCustomLog = async (e) => {
    ...
    try {
      ...
      const currentHistory = loggingRoutine.history || [];
      const updatedHistory = currentHistory.includes(todayString)
        ? currentHistory
        : [...currentHistory, todayString];

      await updateRecurringExpense(loggingRoutine.id, {
  ...
  return (
    <div className="space-y-6">
      <div className="flex sm:items-center justify-between flex-col sm:flex-row gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Manage your recurring transactions that automatically prompt you on
            due dates.
          </p>
        </div>
      ...
      {recurringExpenses && recurringExpenses.length > 0 && (
        <RoutineHeatmap routines={recurringExpenses} />
      )}
      ...
      {!recurringExpenses || recurringExpenses.length === 0 ? (
        <Card className="bg-slate-50 border-dashed dark:bg-slate-900/50">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm mb-4">
              Set up routines for rent, salaries, or daily expenses so you don't
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {recurringExpenses.map((routine) => {
            ...
            const txs = routine.transactions || [
              { amount: routine.amount, type: routine.type || "expense" },
            ];
            ...
            return (
              <Card
                key={routine.id}
                className="overflow-hidden flex flex-col hover:border-indigo-200 dark:hover:border-indigo-800 transition-colors"
              >
                  ...
                  <div className="flex justify-between items-start mb-4 gap-2">
                    <div className="min-w-0">
                      <h3
                        ...
                        title={routine.title || routine.description}
                      >
                  ...
                  <div className="flex items-center justify-between gap-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                    {isLoggedToday ? (
                      ...
                      <div className="group relative">
                        <div
                          className="flex items-center gap-1.5 text-xs font-semibold text-orange-600 dark:text-orange-500 uppercase tracking-wider bg-orange-50 dark:bg-orange-900/20 px-2 py-1 rounded-md cursor-pointer hover:bg-orange-100 transition-colors"
                          onClick={() => handleOpenLogModal(routine)}
                        >
                          <AlertCircle className="w-3.5 h-3.5" /> Skipped
                        </div>
                        <div className="absolute outline-none text-[10px] invisible group-hover:visible left-0 -top-6 bg-slate-800 text-white px-2 py-1 rounded z-10 whitespace-nowrap">
                          Click to force log
                        </div>
                      </div>
                    ) : (
                      <Button
                        ...
                        size="sm"
                        onClick={() => handleOpenLogModal(routine)}
                        className="text-indigo-600 border-indigo-200 hover:bg-indigo-50 hover:border-indigo-300 dark:border-indigo-900/50 dark:text-indigo-400 dark:hover:bg-indigo-900/30 h-8 px-3 transition-colors text-xs font-semibold uppercase tracking-wider"
                      >
                        Log Now
                      </Button>
                    )}
                    ...
                    <div className="flex gap-1 shrink-0">
                      <Button
                        ...
                        className="text-slate-500 hover:text-blue-600 w-8 h-8 rounded-full"
                      >
                      ...
                      <Button
                        ...
                        className="text-slate-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 w-8 h-8 rounded-full"
                      >
        ...
        {!editingRoutine && (
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-800 dark:to-slate-800/80 p-4 mb-4 rounded-xl border border-blue-100 dark:border-slate-700 shadow-sm relative overflow-hidden">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 mb-3">
              Describe all the items for this routine in plain English, and AI
              will build the entire list for you.
            </p>
          ...
          <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            <div className="flex justify-between items-center">
              ...
              <Button
                ...
                className="h-7 gap-1 text-xs px-2 shadow-sm border-slate-200 hover:bg-slate-50"
              >
            ...
            <div className="max-h-60 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
              {transactions.map((tx, idx) => (
                <div
                  key={tx.id}
                  className="p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg space-y-3 relative group transition-colors hover:border-blue-200 dark:hover:border-blue-800"
                >
                  {transactions.length > 1 && (
                    <button
                      ...
                      onClick={() =>
                        setTransactions(
                          transactions.filter((i) => i.id !== tx.id),
                        )
                      }
                      className="absolute top-2 right-2 text-slate-400 hover:text-red-500 opacity-50 group-hover:opacity-100 transition-opacity"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
                  )}
                  <div className="flex gap-2 mb-2 pr-6">
                    <button
                      type="button"
                      className={`flex-1 py-1 rounded-md text-xs font-semibold transition-colors ${tx.type === "expense" ? "bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-400" : "bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-300 dark:hover:bg-slate-600"}`}
                      onClick={() => {
                        const newTx = [...transactions];
                        newTx[idx].type = "expense";
                    ...
                    <button
                      type="button"
                      className={`flex-1 py-1 rounded-md text-xs font-semibold transition-colors ${tx.type === "income" ? "bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400" : "bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-300 dark:hover:bg-slate-600"}`}
                      onClick={() => {
                        const newTx = [...transactions];
                        newTx[idx].type = "income";
                        setTransactions(newTx);
                      }}
                    >
                      Income
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Input
                      ...
                      onChange={(e) => {
                        const newTx = [...transactions];
                        newTx[idx].amount = e.target.value;
                        setTransactions(newTx);
                      }}
                      required
                      placeholder="0.00"
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        const newTx = [...transactions];
                        newTx[idx].description = e.target.value;
                        setTransactions(newTx);
                      }}
                      required
      ...
      <Modal
        isOpen={!!loggingRoutine}
        onClose={() => setLoggingRoutine(null)}
        title="Customize & Log Routine"
      >
        {loggingRoutine && (
          <form onSubmit={handleCustomLog} className="space-y-4">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Tweak the values for this specific run if needed. This doesn't
              change the underlying routine settings.
            </p>
            ...
            <div className="max-h-60 overflow-y-auto space-y-3 pr-2 custom-scrollbar border divide-y divide-slate-100 dark:divide-slate-800 rounded-lg bg-slate-50 dark:bg-slate-900/50">
              {loggingRoutine.txs.map((tx, idx) => {
                const parentCat = tx.category
                  ? tx.category.split(":")[0].trim()
                  : "";
                const subCat =
                  tx.category && tx.category.includes(":")
                    ? tx.category.split(":")[1].trim()
                    : "";
                const parentOptions = categories.map((c) => c.name);
                const subOptions =
                  categories.find((c) => c.name === parentCat)?.subcategories ||
                  [];
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="relative">
                          <select
                            ...
                            className="appearance-none w-full rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 pr-8 py-1.5 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer disabled:opacity-50 text-ellipsis"
                          >
            ...
            <div className="flex justify-between items-center pt-4 border-t border-slate-100 dark:border-slate-800">
              ...
              <div className="flex gap-3">
                <Button
                  variant="ghost"
                  type="button"
                  onClick={() => setLoggingRoutine(null)}
                >
                  Cancel
                </Button>
                <Button
                  type="submit"
                  disabled={isSubmitting || loggingRoutine.txs.length === 0}
                >
