...
export function Settings() {
  ...
  const {
    expenses,
    clearAllData,
    batchUpdateParentCategory,
    batchUpdateSubcategory,
  } = useExpenses();
  const {
    categories,
    addParentCategory,
  ...
  const handleClearData = async () => {
    const isConfirmed = await confirm({
      title: "Danger Zone",
      message:
        "Are you completely sure you want to permanently delete ALL your transactions? This cannot be undone.",
    ...
    if (isConfirmed) {
      ...
      try {
        ...
        addToast(
          "Your data has been successfully reset to default.",
          "success",
        );
      } catch (e) {
    ...
    <div className="space-y-6 max-w-3xl">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center max-md:gap-2 justify-between">
            <div>
              ...
              <p className="text-sm max-md:hidden text-slate-500 dark:text-slate-400">
            ...
            <div className="flex flex-col max-md:gap-3 md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="max-md:hidden text-sm text-slate-500 dark:text-slate-400">
                    {user.email}
                  </p>
                </div>
              </div>
              <Button
                variant="outline"
                onClick={logout}
                className="text-red-600 max-md:text-sm max-md:w-full md:w-auto hover:bg-red-50 hover:text-red-700 dark:text-red-400 dark:hover:bg-red-900/20 dark:border-slate-700"
              >
                Sign Out
              </Button>
            </div>
      ...
      {user && (
        <>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  ...
                  . This key is stored securely in your database and allows the
                  AI agent to parse natural language and receipts.
                </p>
                <div className="flex flex-col max-md:gap-2 gap-2 md:flex-row">
                  <input
                    type="password"
                    value={apiKeyInput}
                  ...
                  <Button
                    ...
                    disabled={
                      isSavingKey || apiKeyInput === preferences?.geminiApiKey
                    }
          ...
          <Card>
            <CardHeader className="flex flex-col max-md:gap-3 md:flex-row md:items-center md:justify-between pb-2">
              <CardTitle className="flex items-center gap-2">
                <Tags className="w-5 h-5 text-blue-600 dark:text-blue-500" />{" "}
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                className="h-8 gap-1 max-md:w-full md:w-auto"
              >
                      ...
                      >
                        <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/50">
                          <button
                            ...
                            className="flex items-center gap-2 flex-1 text-left font-medium text-sm text-slate-900 dark:text-slate-100 focus:outline-none"
                          >
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="p-1.5 text-slate-400 hover:text-green-600 dark:hover:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-md transition-colors"
                              title="Add Subcategory"
                            >
                              <Plus className="w-4 h-4" />
                            </button>
                            <button
                              onClick={() => handleEditParent(cat)}
                            ...
                            <button
                              onClick={() => handleDeleteParent(cat)}
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                      <button
                                        ...
                                        className="p-1 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-md transition-colors"
                                        title="Rename"
                                      >
                                      ...
                                      <button
                                        onClick={() =>
                                          handleDeleteSub(cat, sub)
                                        }
                                        className="p-1 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition-colors"
                                        title="Delete"
                                      >
          ...
          <Card className="border-red-200 dark:border-red-900/50">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-red-600 dark:text-red-500">
                <AlertTriangle className="w-5 h-5" /> Danger Zone Data
                Management
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex flex-col max-md:gap-3 md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <p className="font-medium text-slate-900 dark:text-slate-100">
                    Export Transactions
                  </p>
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    Download a CSV copy of all your tracked expenses.
                  </p>
                </div>
                <Button
                  variant="outline"
                  ...
                  className="shrink-0 gap-2 max-md:w-full md:w-auto"
                >
                  <Download className="w-4 h-4" /> Download Full Record
                </Button>
              </div>

              <div className="flex flex-col max-md:gap-3 md:flex-row md:items-center md:justify-between gap-4 pt-6 border-t border-slate-100 dark:border-slate-800">
                <div>
                  <p className="font-medium text-slate-900 dark:text-slate-100">
                    Reset Database
                  </p>
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    Permanently wipe all your transactions.
                  </p>
                </div>
                <Button
                  variant="outline"
                  onClick={handleClearData}
                  disabled={!user || isClearing}
                  className="shrink-0 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 border-red-200 dark:border-red-900/50 transition-colors max-md:w-full md:w-auto"
                >
